...
function RoundsItem({ rounds }) {
	console.log(rounds);
	return (
		<View key={rounds.id ? rounds.id.toString() : Math.random().toString()} style={{ flex: 1 }}>
			<Text style={{ fontSize: 30, marginHorizontal: 20, marginVertical: 10 }}>{rounds.nome}</Text>
...
export default function MatchPage() {
	...
	useEffect(() => {
		if (
			...
			initialIndex >= 0 &&
			initialIndex < roundsData.length
		) {
	...
	return (
		<ImageBackground
			...
			style={{ flex: 1 }}
		>
			<PagerView initialPage={0} style={{ flex: 1 }} orientation={'horizontal'} ref={pagerRef}>
